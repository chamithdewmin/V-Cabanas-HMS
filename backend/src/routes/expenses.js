...
router.get('/', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      adm ? 'SELECT * FROM expenses ORDER BY created_at DESC' : 'SELECT * FROM expenses WHERE user_id = $1 ORDER BY created_at DESC',
      adm ? [] : [uid]
    );
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const d = req.body;
    const params = [id, d.category, d.amount ? Number(d.amount) : null, d.date, d.paymentMethod, d.isRecurring, d.recurringFrequency, d.continueIndefinitely ? null : (d.recurringEndDate ?? null), d.recurringNotes, d.notes, d.receipt ? JSON.stringify(d.receipt) : null];
    if (adm) {
      await pool.query(
    ...
    const { rows } = await pool.query(
      adm ? 'SELECT * FROM expenses WHERE id = $1' : 'SELECT * FROM expenses WHERE id = $1 AND user_id = $2',
      adm ? [id] : [id, uid]
    );
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const { rowCount } = await pool.query(
      adm ? 'DELETE FROM expenses WHERE id = $1' : 'DELETE FROM expenses WHERE id = $1 AND user_id = $2',
